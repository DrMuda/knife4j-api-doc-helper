...
async function run() {
    ...
    const onTabListChange = debounce(async () => {
        // 找到当前显示的tab
        const activeTabanelList = await waitElement(".knife4j-tab>.ant-tabs-content>div[aria-hidden=false]", 10, 0.5);
...
async function setMyDom(parentDom) {
    ...
    // 设置容器
    const contain = documentDom.querySelector(`.${containClass}`) ||
    ...
    const setTsTypeContent = () => {
        ...
        const requestTypeContain = documentDom.querySelector(`.${requestTypeContainClass}`) ||
            document.createElement("div");
        const responeTypeContain = documentDom.querySelector(`.${responeTypeContainClass}`) ||
            document.createElement("div");
        const copyRequestTypeBtn = (documentDom.querySelector(`.${copyRequestTypeBtnClass}`) || document.createElement("button"));
        const copyResponeTypeBtn = (documentDom.querySelector(`.${copyResponeTypeBtnClass}`) || document.createElement("button"));
        const requestTypeContent = (documentDom.querySelector(`.${requestTypeContentClass}`) || document.createElement("code"));
        const responeTypeContent = (documentDom.querySelector(`.${responeTypeContentClass}`) || document.createElement("code"));
...
// 根据类型创建tsType
function createTsTypeByTarget(targetInnertext, colNumConfig, parentElement) {
...
function createMessage() {
    ...
    messageContain.setAttribute("style", `position: absolute;
    width: fit-content;
    top: 60px;
    left: 50%;
