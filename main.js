...
async function setMyDom(parentDom) {
    // 获取新增Dom的指定位置
    const targetDom = (await waitElement(".knife4j-api-title", 10, 0.5, parentDom))?.[0];
    if (!targetDom) {
    ...
    const exitsTsTypeContain = parentDom.getElementsByClassName(componentId.tsTypeContain)?.[0];
    ...
    const copyRequestTypeBtn = parentDom.getElementsByClassName(id.copyRequestTypeBtn)?.[0];
    const copyResponseTypeBtn = parentDom.getElementsByClassName(id.copyResponseTypeBtn)?.[0];
    const requestTypeContent = parentDom.getElementsByClassName(id.requestTypeContent)?.[0];
    const responseTypeContent = parentDom.getElementsByClassName(id.responseTypeContent)?.[0];
    ...
    const setType = (type) => {
        ...
        const colNumConfig = type === "request"
            ? {
                fieldName: 0,
                annotation: 1,
                isRequired: 3,
        ...
        const typeContent = type === "request" ? requestTypeContent : responseTypeContent;
        const copyTypeBtn = (type === "request" ? copyRequestTypeBtn : copyResponseTypeBtn);
        const successMsg = type === "request" ? "复制请求参数成功" : "复制响应参数成功";
        const trList = table
            .getElementsByTagName("tbody")[0]
            .getElementsByTagName("tr");
        const tree = createTrTree(Array.from(trList));
        let { tsTypeStr, tsTypeStrWithHighlight } = createTsTypeFromTrTree(tree || [], colNumConfig);
...
function getTableDom(type, parentElement) {
    ...
    const titleDomList = Array.from(parentElement.getElementsByClassName("api-title"));
    const requestParamsTitleIndex = titleDomList.findIndex((dom) => dom.innerText === targetInnerText);
    ...
    const tableIdex = targetDomChildren.findIndex((dom) => dom === targetTitleDom) + 1;
...
/** 从树形tr创建tsType字符串 */
function createTsTypeFromTrTree(trTree, 
/** 各数据在哪一列 */
colNumConfig, level = 1) {
    let tsTypeStr = "{\n";
...
async function scrollSelectMenuIntoView() {
    ...
    if (!menu)
        return;
    const selectItem = (await waitElement(".ant-menu-item-selected", 10, 0.5))?.[0];
    if (selectItem) {
        setTimeout(() => {
...
async function generateMenuSearchBar(apiDocs) {
    ...
    Object.entries(paths).forEach(([path, methods]) => {
        Object.entries(methods).forEach(([method, { tags, summary, deprecated }]) => {
            const option = document.createElement("option");
            let innerHTML = `<span class="method method-${method}">${method.toUpperCase()}</span> ${tags.join(" / ")} / ${summary}`;
            if (deprecated === true) {
...
async function replacePath(activeTabPanel) {
    const pathEl = (await waitElement(".knife4j-api-summary-path", 10, 0.5, activeTabPanel));
    if (!pathEl[0])
        return;
...
class OtherPagePathSynchronizer {
    ...
    mount() {
        ...
        syncButton.setAttribute("style", "border:0; color: white; background: #1890ff; border-radius: 4px; cursor: pointer;padding: 2px 4px;");
    ...
    async syncOtherPage() {
        ...
        for (const path of pathList) {
            const currentPageOpenApiTabMap = JSON.parse(sessionStorage.getItem(storageKey.currentPageOpenApiTabMap));
            if (Object.values(currentPageOpenApiTabMap).includes(path))
                continue;
            location.href = path;
            await waitTime(500);
        }
    ...
    responseSync() {
        ...
        if (!key)
            return;
        if (key === this.key.toString())
            return;
        const currentPageOpenApiTabMap = JSON.parse(sessionStorage.getItem(storageKey.currentPageOpenApiTabMap));
        localStorage.setItem(`${storageKey.responseSync}-${this.key}`, JSON.stringify(Object.values(currentPageOpenApiTabMap)));
    }
}
(function () {
    "use strict";
    if (window) {
        interceptApiDocs().then(async (apiDocs) => {
            OtherPagePathSynchronizer.getInstance().mount();
            const tabList = (await waitElement(".knife4j-tab>div[role=tablist]", 10, 0.5))?.[0];
            if (!tabList)
                return;
            scrollSelectMenuIntoView();
            ...
            const onTabListChange = async () => {
                // 找到当前显示的tab
                const activeTabPanelList = await waitElement(".knife4j-tab>.ant-tabs-content>div[aria-hidden=false]", 10, 0.5);
                ...
                const activeTab = document.querySelector(".ant-tabs-top-bar .ant-tabs-tab-active");
                try {
                    const apiTabMap = {
                        [activeTab.innerText]: location.href,
                    };
                    const currentPageOpenApiTabMapStr = sessionStorage.getItem(storageKey.currentPageOpenApiTabMap) ||
                        "{}";
                    ...
                    sessionStorage.setItem(storageKey.currentPageOpenApiTabMap, JSON.stringify(apiTabMap));
                }
                catch (error) {
                    console.error(error);
                }
